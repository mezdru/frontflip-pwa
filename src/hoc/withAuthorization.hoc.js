...
const withAuthorization = (ComponentToWrap, componentName) => {
  ...
  class AuthorizationManagement extends Component {
    ...
    render() {
      ...
      if(isAuth && this.belongsToOrg() && !this.props.commonStore.getCookie('isAccessInLogged')) {
        this.props.commonStore.setCookie('isAccessInLogged', true, (new Date((new Date()).getTime() + 60*60000))) // 1 hour
        this.props.keenStore.recordEvent('accessIn', {recordEmitter: undefsafe(this.props.recordStore.currentUserRecord, '_id')});
      }
