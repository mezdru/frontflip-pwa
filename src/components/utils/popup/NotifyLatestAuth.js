        ...
        <ClickAwayListener onClickAway={this.handleClose}>
          <div className={classes.root}>
            ...
            <Typography variant="h3" className={classes.title}>
              <FormattedMessage id="popup.latestConnection.title" />
            </Typography>
            <Typography variant="body2">
              {coLog && (
                <FormattedMessage
                  id="popup.latestConnection.text"
                  values={{
                    os: coLog.os,
                    browser: coLog.browser,
                    date: moment(coLog.created).calendar(),
                    city: undefsafe(coLog, "location.city"),
                    region: undefsafe(coLog, "location.region")
                  }}
                />
              )}
            </Typography>
...
export default inject(
  "commonStore",
  "authStore"
)(observer(withStyles(style)(NotifyLatestAuth)));
