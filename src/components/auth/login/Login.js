...
class Login extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      authState &&
      authState.action === "signin" &&
      authState.success === "false"
    ) {
      let integrationName = authState.integration
        ? authState.integration.charAt(0).toUpperCase() +
          authState.integration.slice(1)
        : "";
      this.setState({
  ...
  getSigninErrorMessage = err => {
    ...
    if (err.status === 404) {
      ...
      if (err.response.body.error_description === "Wrong password.") {
        ...
        err.response.body.error_description === "User use Google Auth."
      ) {
        errorMessage = this.props.intl.formatMessage({
          id: "signin.error.useGoogle"
        });
      } else {
        errorMessage = this.props.intl.formatMessage(
          { id: "signin.error.noPassword" },
          {
            forgotPasswordLink:
              "/" + this.props.commonStore.locale + "/password/forgot"
          }
      ...
      return (
        <Redirect
          ...
          to={
            "/" +
            locale +
            (currentOrganisation ? "/" + currentOrganisation.tag : "")
          }
        />
      );
    } else {
        ...
        <form onSubmit={this.handleSubmitForm} id="form-login">
          <Grid container item direction="column" spacing={2}>
            ...
            <Grid item justify={"center"} container>
              <Button
                variant="text"
                onClick={() => this.props.handleChangeIndex(1)}
              >
...
export default inject(
  ...
  "orgStore",
  "commonStore"
)(injectIntl(observer(withRouter(Login))));
