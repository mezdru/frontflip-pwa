        ...
        if (this.props.commonStore.url.params.orgTag) {
          this.props.authStore
            ...
            .then(data => {
              ReactGA.event({
                category: "User",
                action: "Register to Organisation"
              });
              ...
              if (cUser && cUser.email && cUser.email.validated) {
                ...
                this.setState({
                  ...
                  registerSuccessMessage: this.props.intl.formatMessage({
                    id: "signup.success"
                  })
                });
              }
            ...
            .catch(err => {
              if (
                err.response &&
                err.response.body &&
                err.response.body.message === "Bad invitation code"
              ) {
                this.setState({ registerWarning: true });
                this.setState({
                  registerWarningMessage: this.props.intl.formatMessage(
                    { id: "signup.warning.badInvitationCode" },
                    { orgName: this.props.orgStore.currentOrganisation.name }
                  )
                });
              } else {
                ...
                this.setState({
                  registerSuccessMessage: this.props.intl.formatMessage(
                    { id: "signup.warning.forbiddenOrg" },
                    { orgName: this.props.orgStore.currentOrganisation.name }
                  )
                });
              }
          ...
          err.response.body.errors.forEach(error => {
            if (error.param === "password") {
              if (error.type === "dumb") {
                ...
                errorMessage =
                  ...
                  this.props.intl.formatMessage(
                    { id: "signup.error.dumbPassword" },
                    {
                      dumbCount: (parseInt(error.msg) * 30000).toLocaleString()
                    }
                  );
              } else {
                errorMessage =
              ...
              errorMessage =
                (errorMessage ? errorMessage + "<br/>" : "") +
                this.props.intl.formatMessage({
                  id: "signup.error.wrongEmail"
                });
            }
          });
        } else if (
          err.status === 400 &&
          err.response.body.message === "User already exists."
        ) {
          errorMessage = this.props.intl.formatMessage(
            { id: "signup.error.userExists" },
            {
              forgotPasswordLink:
                "/" + this.props.commonStore.locale + "/password/forgot"
            }
          );
        }
      ...
      return (
        <Grid container item direction="column" spacing={2}>
          <Grid item>
            <SnackbarCustom
              variant="warning"
              message={registerWarningMessage}
            />
          </Grid>
        </Grid>
      );
        ...
        <form onSubmit={this.handleSubmitForm} id="form-register">
          <Grid container item direction="column" spacing={2}>
            ...
            <Grid item container justify="center">
              <Button
                variant="text"
                onClick={() => this.props.handleChangeIndex(0)}
              >
            ...
            {currentAuthProviders.length > 0 && (
              <>
                <Grid item>
                  ...
                  >
                    <FormattedHTMLMessage id="signup.or.integration" />
                  </Typography>
                </Grid>
                <Grid
                  container
